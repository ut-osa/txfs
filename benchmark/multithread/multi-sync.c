...
void *start_sync(void *ptr) {
	int i;

	bind_core((NUM_READ_THREAD + NUM_WRITE_THREAD) % num_cores);
...
int main(void) {
  ...
  ret = pthread_create(&sync_thread, NULL, start_sync, NULL);
  if (ret != 0) printf("Error starting sync thread %d\n", i);
  ...
  printf("sync pthread %d finished\n", i);
  if (ret != 0) printf("Error joining sync thread %d\n", i);
